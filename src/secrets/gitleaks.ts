...
/**
 * gitleaks adapter, plus the shared "gitleaks-like" CLI driver reused by the
 * betterleaks adapter (betterleaks is a gitleaks successor by the same team and
 * exposes the same `stdin -f json` interface and report JSON shape).
 *
 * `gitleaks stdin` (v8.19.3+ for our flags) scans piped content with no temp
...
/** Parse `gitleaks`/`betterleaks` `-f json` report output. Exported for fixture tests. */
export function parseGitleaksOutput(stdout: string, scanner: SecretScannerName = "gitleaks"): SecretFinding[] {
...
/** Run a gitleaks-compatible CLI (`gitleaks` or `betterleaks`) over stdin. */
export async function scanWithGitleaksCli(
  bin: string,
  ...
  try {
    ...
    try {
      ...
      throw new ScanUnavailableError(bin, `unparseable report: ${(err as Error).message}`);
    }
  } finally {
