#!/usr/bin/env bash
# Fencepost PostToolUse hook
# Reads JSON from stdin; when secrets scanning is enabled, redacts secrets from
# the tool output via updatedToolOutput. Emits nothing when there is no work.
...
exec "$RUNNER" "$HERE/../dist/index.js" posttooluse
