...
function unquoteText(node) {
  switch (node.type) {
    ...
    case "concatenation": {
      let s = "";
      for (let i2 = 0;i2 < node.namedChildCount; i2++)
        s += unquoteText(node.namedChild(i2));
      return s;
    }
    default:
...
import { resolve as resolve2, dirname as dirname3, basename } from "node:path";
...
function inlineCodeOf(cmd) {
  for (let i2 = 0;i2 < cmd.args.length - 1; i2++) {
    if (CODE_FLAGS.has(cmd.args[i2]))
      return cmd.args[i2 + 1];
  }
  return cmd.heredoc;
}
    ...
    if (n.type === spec.callNode) {
      ...
      if (argsNode) {
        for (let i2 = 0;i2 < argsNode.namedChildCount; i2++) {
          const a = argsNode.namedChild(i2);
...
function shellWrapperCode(cmd) {
  ...
  if (cmd.name && SHELL_WRAPPERS.has(cmd.name)) {
    const i2 = cmd.args.indexOf("-c");
    if (i2 !== -1 && i2 + 1 < cmd.args.length)
      return cmd.args[i2 + 1] || null;
  }
  return null;
}
...
// src/config.ts
import { join as join9, resolve as resolve3, dirname as dirname5 } from "node:path";
...
function presetSearchDirs2() {
  ...
  if (envDir)
    dirs.push(envDir);
  try {
    dirs.push(join9(dirname5(process.execPath), "..", "presets"));
  } catch {}
...
async function loadImports2(names, importedFrom) {
  ...
  for (const name2 of await expandImports2(names)) {
    if (name2 === "user") {
      const layer = await loadCandidateLayer2(userConfigCandidate2());
      if (!layer) {
        note2("warning", importedFrom, "imported 'user' but no user-level config was found");
        continue;
      }
      merged = mergeConfigs2(merged, layer.config);
...
class CompiledConfig2 {
  ...
  get errors() {
    return this.issues.filter((i2) => i2.level === "error");
  }
  get warnings() {
    return this.issues.filter((i2) => i2.level === "warning");
  }
  get ok() {
...
function userConfigCandidate2() {
  const home = process.env["FENCEPOST_HOME"] || homedir4();
  return { confDir: join9(home, ".claude", "fencepost", "config"), singleFile: join9(home, ".claude", "fencepost.yaml") };
}
async function loadCandidateLayer2(candidate) {
  const dirResult = await loadConfDir2(candidate.confDir);
  if (dirResult)
    return dirResult;
  if (existsSync3(candidate.singleFile)) {
    const loaded = await loadYamlFile2(candidate.singleFile);
    return {
...
async function resolveInternal2(cwd) {
  ...
  const candidates2 = [
    ...
    userConfigCandidate2()
  ];
  let host = null;
  for (const candidate of candidates2) {
    const result = await loadCandidateLayer2(candidate);
    if (result) {
...
`;
    await mkdir(dirname4(logPath), { recursive: true });
